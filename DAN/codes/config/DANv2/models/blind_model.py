...
class PCAdecoder(nn.Module):
    ...
    def forward(self, batch_kernel):
        B, _ = batch_kernel.size()  # [B, l, l]
        
        a = batch_kernel.unsqueeze(0)
        b = torch.transpose(self.weight, 0, 1).unsqueeze(0)
        
        results = torch.bmm(a, b).view(B, 21, 21)
        return results
...
class B_Model(BaseModel):
    def __init__(self, opt):
        ...
        pca_matrix = torch.load(
            self.opt["pca_matrix_path"], map_location=lambda storage, loc: storage
        )
        
        self.decoder = PCAdecoder(pca_matrix).cuda()

        self.degradation_model = DegradationModel(opt['ksize'], opt['scale'], sv_degradation=True)
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            self.fake_SR = srs[-1]
            self.kernels = kernels[-1]
            self.fake_ker = self.decoder(kermaps[-1])
            # self.fake_ker = kernels[-1]
            self.fake_LR = self.degradation_model(self.real_H, self.fake_ker)
    ...
    def get_current_visuals(self):
        ...
        out_dict["LLR"] = self.fake_LR.detach()[0].float().cpu()
